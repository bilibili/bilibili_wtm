...
#             self.image_list.append(Image.open(os.path.join(self.path, img_name)).convert("RGB"))
...
class MBRSTestDataset(Dataset):

    def __init__(self, path, H=256, W=256):
        super(MBRSTestDataset, self).__init__()
        self.H = H
        self.W = W
        self.path = path
...
class MBRSValDataset(Dataset):

    def __init__(self, path, H=256, W=256):
        super(MBRSValDataset, self).__init__()
        self.H = H
        self.W = W
        self.path = path
        self.list = sorted(os.listdir(path))
