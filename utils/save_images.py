...
def save_per_frame(images, frame, folder):
    ...
    images = ((images.clamp(-1, 1).permute(1, 2, 0) + 1) / 2 * 255).to('cpu', torch.uint8).numpy()
    # saved_image = Image.fromarray(images)
    saved_image = images[:, :, ::-1]
    # print(saved_image.sum())

    filename = os.path.join(folder, 'frame-%04d.png' % frame)

    # saved_image.save(filename)
    cv2.imwrite(filename, saved_image)


def save_crf_img(epoch, original_path, encode_path, img_name='frame-0000.png'):
    cover_img = cv2.imread(os.path.join(original_path, img_name))
    encode_img = cv2.imread(os.path.join(encode_path, 'encode', img_name))
    diff_img = (cover_img.astype(np.float32) - encode_img.astype(np.float32)) + 127.5
    norm_img = (diff_img - diff_img.min()) / (diff_img.max() - diff_img.min()) * 255
    all_img = np.concatenate((cover_img, encode_img, diff_img, norm_img), 1)
    cv2.imwrite(os.path.join(encode_path, 'images', 'epoch-{}-'.format(epoch) + img_name), all_img)
