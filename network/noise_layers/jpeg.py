...
class JpegTest(nn.Module):
    ...
    def get_path(self):
        # return self.path + ''.join(random.sample(string.ascii_letters + string.digits, 16)) + ".png"
    ...
    def forward(self, image_and_cover):
        ...
        # print(single_image.sum(),image.sum(), noised_image.sum(), cover_image.sum())
...
class Jpeg(JpegBasic):
    ...
    def forward(self, image_and_cover):
        if isinstance(self.Q, list):
            rand_Q = np.random.uniform(self.Q[0], self.Q[1])
            self.scale_factor = 2 - rand_Q * 0.02 if rand_Q >= 50 else 50 / rand_Q
...
class JpegTwice(JpegBasic):
    def __init__(self, Q, subsample=0):
        super(JpegTwice, self).__init__()
    ...
    def jpeg_real(self, image_and_cover):
        if isinstance(self.Q, list):
            rand_Q = np.random.uniform(self.Q[0], self.Q[1])
            self.scale_factor = 2 - rand_Q * 0.02 if rand_Q >= 50 else 50 / rand_Q
        assert self.scale_factor >= 0
    ...
    def jpeg_mask(self, image_and_cover):
        image, cover_image = image_and_cover
    ...
    def jpeg_ss(self, image_and_cover):
        if isinstance(self.Q, list):
            rand_Q = np.random.uniform(self.Q[0], self.Q[1])
            self.scale_factor = 2 - rand_Q * 0.02 if rand_Q >= 50 else 50 / rand_Q
        assert self.scale_factor >= 0
    ...
    def identity(self, image_and_cover):
        image, cover_image = image_and_cover
        return image

    def forward(self, image_and_cover):
        encode_image, cover_image = image_and_cover
        attack_pool = [self.jpeg_real, self.jpeg_mask, self.identity]
        attack_type1, attack_type2 = np.random.choice(attack_pool, 2)

        attack1 = attack_type1([encode_image, cover_image])
        attack2 = attack_type2([attack1, cover_image])
        return attack2
